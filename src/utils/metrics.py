...
def _aggregate_loads(load_ids, y_true, y_pred, metric_fn, n_iters=100):
    ...
    return np.mean([x[0] for x in errors]), np.sqrt(sum(map(lambda x: x[1]**2,errors)))
...
def mean_estimation_absolute_percentage_error(load_ids, y_true, y_pred, n_iters=100):   
    def metric_fn(y_true, y_pred, eps=1e-8):
