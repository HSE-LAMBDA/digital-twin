...
def aggregate_loads(load_ids, y_true, y_pred, metric_fn, n_iters=100):
    errors = []
...
def mean_estimation_absolute_percentage_error(y_true, y_pred, eps=1e-8):   
    return np.abs((y_true.mean(0)-y_pred.mean(0))/(eps+y_true.mean(0))).squeeze()*100

def std_estimation_absolute_percentage_error(y_true, y_pred, eps=1e-8):
