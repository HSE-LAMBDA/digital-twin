...
class GMM:
        ...
        if self.use_elliptic_envelope:
            try:
                self.elliptic_envelope = EllipticEnvelope(
                    contamination=self.elliptic_envelope_contamination,
    ...
    def init_from_params(self, weights, means, precisions_cholesky):
        ...
        self.model.precisions_cholesky_ = self.triu_to_full_precisions_cholesky(
            precisions_cholesky
        )
        # need to be more generic
        covariances = np.linalg.inv(
            self.model.precisions_cholesky_
            @ np.transpose(self.model.precisions_cholesky_, (0, 2, 1))
        )
    ...
    def triu_to_full_precisions_cholesky(self, upper_triangular_elements):
        ...
        n = np.floor(
            (np.sqrt(1 + 8 * upper_triangular_elements.shape[1]) - 1) / 2
        ).astype(int)
